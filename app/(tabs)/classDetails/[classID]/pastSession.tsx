...
import { 
  ...
  exportAllClassesAndSessions, 
  exportAllSessionsToZip 
} from '../../../../components/capstone/CSVExports';
...
export default function PastSessionViewer() {
  ...
  const [teacherClasses, setTeacherClasses] = useState<{ class_id: string; class_name: string }[]>([]);
  const [selectedClass, setSelectedClass] = useState<string | null>(class_id);
    ...
    const fetchSessions = async () => {
      ...
      try {
        if (showAllClasses) {
          ...
          let sessionArray: ExtendedSessionInfo[] = [];
    
          // Fetch sessions from every class
          ...
          const sortedSessions = sessionArray.sort((a, b) =>
            new Date(b.generated_local).getTime() - new Date(a.generated_local).getTime()
          );
    
          setAllSessions(sortedSessions);
          setSessions(sortedSessions.slice(0, 10));
        } else {
          // Fetch sessions for single selected class
          const resSessions = await fetch(`${API_URL}/classes/${class_id}/sessions`, {
            headers: { Authorization: `Bearer ${token}` },
          });
          ...
          const sessionArray = await resSessions.json();
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={{ alignItems: 'center', marginVertical: 10 }}>
        ...
        >
          <Text style={{ color: '#7C3AED', fontSize: 16, fontWeight: '500' }}>
            {showAllClasses ? 'Viewing all classes — Click to view selected class' : 'Viewing single class — Click to view all classes'}
          </Text>
        </TouchableOpacity>
      </View>
      ...
      {showAllClasses && teacherClasses.length > 0 && (
        <View style={{ alignItems: 'center', marginVertical: 10 }}>
          {teacherClasses.map((cls) => (
            <TouchableOpacity
              key={cls.class_id}
              onPress={() => setSelectedClass(cls.class_id)}
              style={{ marginVertical: 6 }}
            >
              <Text style={{ fontSize: 16, color: selectedClass === cls.class_id ? '#4C1D95' : '#1E3A8A' }}>
                {cls.class_name}
              </Text>
            </TouchableOpacity>
          ))}
      ...
      <View style={{ alignItems: 'center', marginVertical: 12 }}>
        <TouchableOpacity onPress={() => setExportModalVisible(true)}>
          <Text style={{ color: '#1E3A8A', fontSize: 16, fontWeight: '500' }}>
            📤 Export Options
          </Text>
        </TouchableOpacity>
      </View>
      ...
      >
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#00000099',}}>
          <View style={{ backgroundColor: 'white', padding: 24, borderRadius: 12, width: '25%' }}>
            ...
            <TouchableOpacity onPress={async () => {
              const token = await AsyncStorage.getItem('token');
              if (!token) {
                showAlert('Error', 'Missing token. Please log in again.');
                return;
              }
              if (!selectedSession) {
                showAlert('Error', 'No session selected to export.');
                return;
              }
              await exportAttendanceToCSV(attendance, selectedSession.class_name, new Date(selectedSession.generated_at).toISOString().split('T')[0]);
              showAlert('Success', 'Export completed.');
              setExportModalVisible(false);
            }}>
              <Text style={{ color: '#1E3A8A', fontSize: 16, marginBottom: 12, textAlign: 'center' }}>
                📄 Export This Session
              </Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity onPress={async () => {
              ...
              const sessionsForSelectedClass = allSessions.filter((s) => s.class_name === className);
              await exportAllSessionsToCSV(sessionsForSelectedClass, token, className);
              showAlert('Success', 'Export completed.');
              setExportModalVisible(false);
            }}>
              <Text style={{ color: '#1E3A8A', fontSize: 16, marginBottom: 12, textAlign: 'center' }}>
            ...
            {showAllClasses && (
              <>
                <TouchableOpacity onPress={async () => {
                  ...
                  await exportAllClassesAndSessions(teacherClasses, token);
                  showAlert('Success', 'Export completed.');
                  setExportModalVisible(false);
                }}>
                  <Text style={{ color: '#B91C1C', fontSize: 16, marginBottom: 12, textAlign: 'center' }}>
                    📄 Export All Classes and Sessions
                  </Text>
                </TouchableOpacity>
                ...
                <TouchableOpacity onPress={async () => {
                  ...
                  const confirmExport = window.confirm('Are you sure you want to export all sessions as a ZIP file? This might take a while.');
                  if (!confirmExport) return;

                  await exportAllSessionsToZip(allSessions, token);
                  showAlert('Success', 'Export completed.');
                  setExportModalVisible(false);
                }}>
                  <Text style={{ color: '#B91C1C', fontSize: 16, marginBottom: 12, textAlign: 'center' }}>
                    🗜️ Export All Sessions as ZIP
                  </Text>
                </TouchableOpacity>
              </>
            )}
