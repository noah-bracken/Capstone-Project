    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={{ alignItems: 'center', marginVertical: 12 }}>
        <TouchableOpacity onPress={() => setShowRemoveModal(true)}>
          <Text style={{ color: '#B91C1C', fontSize: 16, fontWeight: '500' }}>
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalContainer}>
          ...
          <Text style={styles.modalText}>Are you sure you want to remove this student?</Text>

          <View style={styles.buttonContainer}>
            <TouchableOpacity style={styles.cancelButton} onPress={() => setShowRemoveModal(false)}>
              <Text style={styles.buttonText}>Cancel</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.confirmButton} onPress={async () => {
                  try {
                    const token = await AsyncStorage.getItem('token');
                    const res = await fetch(`${API_URL}/classes/${class_id}/students/${studentID}`, {
                      method: 'DELETE',
                      headers: {
