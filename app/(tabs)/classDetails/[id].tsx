...
export default function ClassScreen() {
  ...
  const [studentAttendanceRates, setStudentAttendanceRates] = useState<{ [key: number]: number }>({});
  ...
  const fetchAllStudentAttendanceRates = async () => {
    if (!sessionToken || !id || !classData?.students) return;
  
    const newRates: { [key: number]: number } = {};
  
    for (const student of classData.students) {
      try {
        const response = await fetch(`${API_URL}/students/${student.user_id}/attendance/summary?class_id=${id}`, {
          headers: {
            Authorization: `Bearer ${sessionToken}`,
          },
        });
        const data = await response.json();
        newRates[student.user_id] = data.totalAttendance;
      } catch (error) {
        console.error(`Error fetching attendance for student ${student.user_id}:`, error);
      }
    }
  
    setStudentAttendanceRates(newRates);
  };
...
useFocusEffect(
  useCallback(() => {
    ...
    if (role === 'teacher' && classData?.latest_qr_id) {
      ...
      fetchAllStudentAttendanceRates();
    }
              ...
              {!classData.students || classData.students.length === 0 ? (
                ...
                [...classData.students]
                  ...
                  .map((student) => (
                    ...
                    >
                      <View style={styles.lowAttendance}>
                        <Text style={styles.studentName}>
                          {student.first_name} {student.last_name}
                        </Text>
                        {studentAttendanceRates[student.user_id] !== undefined && studentAttendanceRates[student.user_id] < 75 && (
